    ...
    def test_socket_protocol(self):
        ...
        for case in testcases:
            ...
            if case.expectedSocketProtocol == None:
                ...
                errs_list = list(errors.values())

                # note: this could be flaky if other future changes causes other errors during listener setup
                # this does make the assumption that no default listeners are added and the first
                # error is the one for setting socket_protocol
                assert case.name in errs_list[0][0]['error'] 
